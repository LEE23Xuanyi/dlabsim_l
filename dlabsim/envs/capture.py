...
from dlabsim.envs.simulator import BaseConfig
...
class CaptureBase(SimulatorBase):
    def __init__(self, config: CaptureCfg):
        ...
        # self.save_dir = os.path.join(DLABSIM_ROOT_DIR, f"data/capture/exhibition/{config.expreriment}")
        self.save_dir = os.path.join(DLABSIM_ROOT_DIR, f"data/iros_talk/{config.expreriment}")
