...
class SimNode(AirbotPlayBase):
    def resetState(self):
        ...
        self.mj_data.qpos[self.nj+8] += -np.random.random() * 0.05 + 0.025
        self.mj_data.qpos[self.nj+9] += -np.random.random() * 0.05 + 0.025
...
if __name__ == "__main__":
    ...
    cfg = AirbotPlayCfg()
    cfg.mjcf_file_path = "mjcf/3blocks_stack.xml"
    cfg.rb_link_list = ["arm_base", "link1", "link2", "link3", "link4", "link5", "link6", "right", "left"]
    cfg.obj_list     = ["block_purple", "block_yellow","block_red"]
    cfg.sync         = False
    cfg.headless     = False
    cfg.decimation   = 4
    cfg.render_set   = {
    ...
    save_dir = os.path.join(DLABSIM_ROOT_DIR, "data/3blocks_stack")
    ...
    while sim_node.running:
        try:
            ...
            elif state_idx == 8: # move the block_red to block_yellow_above
            ...
            elif state_idx == 9: # moving the block_red to block_yellow_above
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1

            elif state_idx == 10: # move the block_red to block_yellow
                posi, quat = obs["obj_pose"]["block_yellow"]
                Tmat_block_g_global = np.eye(4)
                Tmat_block_g_global[:3,:3] = Rotation.from_quat(quat[[1,2,3,0]]).as_matrix()
                Tmat_block_g_global[:3, 3] = posi
                Tmat_move_bias[2,3] = 0.0725
                Tmat_block_g_local = Tmat_move_bias @ Tmat_base_inv @ Tmat_block_g_global
            ...
            elif state_idx == 11: # moving the block_red to block_yellow
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1                
            ...
            elif state_idx == 14: # move to mid
                posi = np.array([-0.08, 0, 0.724])
            ...
            elif state_idx == 15: # moving to mid
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    
                    state_idx += 1
            

            # purple block stacking

            if state_idx == 16: # move to block_purple above
                posi, quat = obs["obj_pose"]["block_purple"]
                Tmat_block_g_global = np.eye(4)
                Tmat_block_g_global[:3,:3] = Rotation.from_quat(quat[[1,2,3,0]]).as_matrix()
                Tmat_block_g_global[:3, 3] = posi
                Tmat_move_bias[2,3] = 0.13
            ...
            elif state_idx == 17: # moving to block_purple above
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1

            elif state_idx == 18: # move to block_purple
                posi, quat = obs["obj_pose"]["block_purple"]
                Tmat_block_g_global = np.eye(4)
                Tmat_block_g_global[:3,:3] = Rotation.from_quat(quat[[1,2,3,0]]).as_matrix()
                Tmat_block_g_global[:3, 3] = posi
            ...
            elif state_idx == 19: # moving to block_purple
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1
            ...
            elif state_idx == 22: # pickup block_purple
                posi, quat = obs["obj_pose"]["block_purple"]
                Tmat_block_g_global = np.eye(4)
                Tmat_block_g_global[:3,:3] = Rotation.from_quat(quat[[1,2,3,0]]).as_matrix()
                Tmat_block_g_global[:3, 3] = posi
            ...
            elif state_idx == 23: # picking up block_purple
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1

            elif state_idx == 24: # move the block_purple to block_red_above
                posi, quat = obs["obj_pose"]["block_red"]
                Tmat_block_g_global = np.eye(4)
                Tmat_block_g_global[:3,:3] = Rotation.from_quat(quat[[1,2,3,0]]).as_matrix()
                Tmat_block_g_global[:3, 3] = posi
            ...
            elif state_idx == 25: # moving the block_red to block_red_above
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1


            elif state_idx == 26: # move the block_purple to block_red
                posi, quat = obs["obj_pose"]["block_red"]
                Tmat_block_g_global = np.eye(4)
                Tmat_block_g_global[:3,:3] = Rotation.from_quat(quat[[1,2,3,0]]).as_matrix()
                Tmat_block_g_global[:3, 3] = posi
            ...
            elif state_idx == 27: # moving the block_purple to block_red
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1     
            ...
            elif state_idx == 30: # init
                tarjq = init_control.copy()
                
                state_idx += 1

            elif state_idx == 31: # initing
                if np.allclose(obs["jq"][:6], tarjq[:6], atol=1e-2):
                    state_idx += 1
        ...
        if state_idx > 31:
            posi_1, quat_1 = obs["obj_pose"]["block_yellow"]
            posi_2, quat_2 = obs["obj_pose"]["block_red"]
            posi_3, quat_3 = obs["obj_pose"]["block_purple"]
            suc1 = np.linalg.norm((posi_1 - posi_2)[:2]) < 0.009
            suc2 = np.linalg.norm((posi_1 - posi_3)[:2]) < 0.009
            if not suc1 or not suc2:
                print(f"\n{data_idx} fail")
            else:

                save_path = os.path.join(save_dir, "{:03d}{}".format(data_idx, "" if suc1 and suc2 else "_fail"))
