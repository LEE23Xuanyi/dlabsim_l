...
class MMK2ExhibitionWithSM(SuperStateMachine):
    ...
    def run(self):
        ...
        while self.mmk2.running:
            # self.mmk2.sysc_real_robot(False)
            self.mmk2.view()
        thr_stmt.join()
    ...
    def pickup_one_prize(self, prize_id):
        ...
        if self.last_prize_id is None or (not self.last_prize_id is None and self.last_prize_id != prize_id):
            self.mmk2.action_normal(only_arm=True, nop=False)
...
if __name__ == "__main__":
    ...
    # b2 = Box(number=4, prize_type="second_prize", status="on_desk", box_position=[1, 3])
    # b3 = Box(number=4, prize_type="third_prize" , status="on_desk", box_position=[1, 2])
