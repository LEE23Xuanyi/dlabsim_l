...
class SimNode(AirbotPlayBase):
    def resetState(self):
        ...
        self.mj_data.qpos[self.nj+1] =  np.random.random() * 0.2  - 0.1
        self.mj_data.qpos[self.nj+2] = -np.random.random() * 0.15 + 0.1

        self.mj_data.qpos[self.nj+8] =  np.random.random() * 0.2  - 0.1
        self.mj_data.qpos[self.nj+9] = -np.random.random() * 0.15 - 0.1
    ...
    def getObservation(self):
        ...
        obj_pose["camera"] = self.getCameraPose(self.config.obs_camera_id)
...
if __name__ == "__main__":
    ...
    cfg = AirbotPlayCfg()
    cfg.mjcf_file_path = "mjcf/il_icra.xml"
    ...
    save_dir = os.path.join(DLABSIM_ROOT_DIR, "data/act_airbot_play_gs_2")
    ...
    while sim_node.running:
        ...
        if state_idx > 10:
            posi_1, quat_1 = obs["obj_pose"]["cup_pink"]
            posi_2, quat_2 = obs["obj_pose"]["cup_blue"]
